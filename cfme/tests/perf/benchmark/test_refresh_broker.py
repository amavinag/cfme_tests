...
pytestmark = [
    pytest.mark.usefixtures('patch_rails_console_use_vim_broker', 'patch_broker_cache_scope',
        'end_log_benchmark', 'end_pbench_move_results')
]

benchmark_values = OrderedDict()
test_run_ts = '{}-Refresh-Broker'.format(str(datetime.datetime.now()).replace(" ", "_"))
...
@pytest.mark.parametrize('iteration',
    range(0, perf_tests['benchmark']['refresh_broker']['provider_init']))
@pytest.mark.parametrize('provider', get_benchmark_vmware_providers())
def test_refresh_broker_provider_init(ssh_client, clean_appliance, iteration, provider):
    ...
    test_iteration = 'Provider-Broker-Init-{}-{}'.format(provider_name, str(iteration).zfill(4))
    ...
    parse_benchmark_output(output, 'Refresh', 'Provider-Broker-Init', provider_name, iteration,
        benchmark_values)
...
@pytest.mark.parametrize('iteration',
    range(0, perf_tests['benchmark']['refresh_broker']['provider_init']))
@pytest.mark.parametrize('provider', get_benchmark_vmware_providers())
def test_profile_refresh_broker_provider_init_perftools(ssh_client, clean_appliance, iteration,
        provider):
    ...
    try:
        ...
        logger.error('Unexpected Output: {}'.format(output))
    append_value(benchmark_values, 'Refresh', 'Provider-Broker-Init-Perftools', provider_name,
        'timing', float(output.strip().split('\n')[-1]))
...
@pytest.mark.parametrize('iteration',
    range(0, perf_tests['benchmark']['refresh_broker']['provider_init']))
@pytest.mark.parametrize('provider', get_benchmark_vmware_providers())
def test_profile_refresh_broker_provider_init_stackprof(ssh_client, clean_appliance, iteration,
        provider):
    """Measures time required to complete an initial EmsRefresh.refresh on specific provider and
    ...
    try:
        ...
        logger.error('Unexpected Output: {}'.format(output))
    append_value(benchmark_values, 'Refresh', 'Provider-Broker-Init-Stackprof', provider_name,
        'timing', float(output.strip().split('\n')[-1]))
    logger.info('Iteration: {}, Value: {}'.format(iteration, output.strip().split('\n')[-1]))
...
def test_refresh_broker_provider_delta(ssh_client, clean_appliance, provider):
    ...
    provider_name = cfme_data['management_systems'][provider]['name']
    for iteration in range(perf_tests['benchmark']['refresh_broker']['provider_delta']):
        test_iteration = 'Provider-Broker-Delta-{}-{}'.format(provider_name,
        ...
        try:
            ...
            pbench_stop(ssh_client, test_run_ts, test_iteration, results=output)
        parse_benchmark_output(output, 'Refresh', 'Provider-Broker-Delta', provider_name, iteration,
            benchmark_values)
...
def test_profile_refresh_broker_provider_delta_perftools(ssh_client, clean_appliance, provider):
    ...
    """
    setup_test_refresh(provider, True, ssh_client)
    iterations = perf_tests['benchmark']['refresh_broker']['provider_delta']
    provider_name = cfme_data['management_systems'][provider]['name']
    command = ('e = ExtManagementSystem.find_by_name(\'' + provider_name + '\');'
               'require \'perftools\';'
               'r = Array.new;'
    ...
    exit_status, output = ssh_client.run_rails_console(command, timeout=None)
    try:
        parse_benchmark_array(output, 'Refresh', 'Provider-Broker-Delta-Perftools', provider_name)
    except ValueError:
        logger.error('Unexpected Output: {}'.format(output))
        assert 0
...
def test_profile_refresh_broker_provider_delta_stackprof(ssh_client, clean_appliance, provider):
    ...
    exit_status, output = ssh_client.run_rails_console(command, timeout=None)
    try:
        parse_benchmark_array(output, 'Refresh', 'Provider-Broker-Delta-Stackprof', provider_name)
    except ValueError:
        logger.error('Unexpected Output: {}'.format(output))
        assert 0
...
def test_refresh_broker_host(ssh_client, clean_appliance, provider):
    ...
    for iteration in range(perf_tests['benchmark']['refresh_broker']['host']):
        test_iteration = 'Host-Broker-{}-{}'.format(provider_name, str(iteration).zfill(4))
        pbench_start(ssh_client, test_run_ts, test_iteration)
        exit_status, output = ssh_client.run_rails_console(code, timeout=None)
        try:
            float(output.strip().split('\n')[-1])
            ...
            pbench_stop(ssh_client, test_run_ts, test_iteration, results=output)
        parse_benchmark_output(output, 'Refresh', 'Host-Broker', provider_name, iteration,
            benchmark_values)


@pytest.mark.parametrize('provider', get_benchmark_vmware_providers())
def test_refresh_broker_vm(ssh_client, clean_appliance, provider):
    """Measures time required to complete an EmsRefresh.refresh on specific VM(s) after initial
    refresh."""
    setup_test_refresh(provider, True, ssh_client)
    provider_name = cfme_data['management_systems'][provider]['name']
    num_vms = perf_tests['benchmark']['refresh_broker']['vm_targets']
    for vms in num_vms:
        ...
        for iteration in range(perf_tests['benchmark']['refresh_broker']['vm']):
            test_iteration = 'VM-Broker-{}-{}-{}'.format(vms, provider_name,
                str(iteration).zfill(4))
            pbench_start(ssh_client, test_run_ts, test_iteration)
            exit_status, output = ssh_client.run_rails_console(code, timeout=None)
            try:
                ...
                pbench_stop(ssh_client, test_run_ts, test_iteration, results=output)
            parse_benchmark_output(output, 'Refresh', 'VM-Broker-{}'.format(vms), provider_name,

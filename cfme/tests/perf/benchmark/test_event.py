...
from utils.perf import log_benchmark_statistics
from utils.perf import log_benchmark
from utils.perf import pbench_start, pbench_stop
from utils.perf import set_server_roles_benchmark_event
...
test_run_ts = '{}-Event'.format(str(datetime.datetime.now()).replace(" ", "_"))
...
def test_vmware_event(ssh_client, clean_appliance, provider, event):
    ...
    set_server_roles_benchmark_event()
    ...
    command = 'e = ExtManagementSystem.find_by_name(\'{}\')\n{}'.format(provider_name,
        event_gen[event])
    timings = []
    for iteration in range(perf_tests['benchmark']['event']['vmware'][event]):
        test_iteration = 'Event-{}-{}'.format(event, str(iteration).zfill(4))
    ...
    log_benchmark_statistics(timings, 'Event', event, provider_name, 'timing')

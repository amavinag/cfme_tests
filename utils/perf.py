...
def fast_add_provider(provider, ssh_client):
    """Use ruby code to create a provider really fast."""
    pro_name = cfme_data['management_systems'][provider]['name']
    pro_hostname = cfme_data['management_systems'][provider]['hostname']
    pro_username = credentials[cfme_data['management_systems'][provider]['credentials']]['username']
    pro_password = credentials[cfme_data['management_systems'][provider]['credentials']]['password']

    if version.current_stream() == 'downstream-52z':
        if cfme_data['management_systems'][provider]['type'] == 'virtualcenter':
            pro_type = 'EmsVmware'
        elif cfme_data['management_systems'][provider]['type'] == 'rhevm':
            pro_type = 'EmsRedhat'
        command = ('e = ExtManagementSystem.create(:name => \\\"' + pro_name +
                '\\\", :hostname => \\\"' + pro_hostname + '\\\", :ipaddress => \\\"' + pro_hostname
                + '\\\", :zone => Zone.default_zone, :type => \\\"' + pro_type + '\\\");'
                'e.update_authentication(:default => {:userid => \\\"' + pro_username +
                '\\\", :password => \\\"' + pro_password + '\\\"});'
                'e.authentication_check;')
    elif version.current_stream() == 'downstream-55z':
        if cfme_data['management_systems'][provider]['type'] == 'virtualcenter':
            pro_type = 'ManageIQ::Providers::Vmware::InfraManager'
        elif cfme_data['management_systems'][provider]['type'] == 'rhevm':
            pro_type = 'ManageIQ::Providers::Redhat::InfraManager'
        command = ('e = ManageIQ::Providers::InfraManager.create( :name => \\\"' + pro_name +
                '\\\", :hostname => \\\"' + pro_hostname + '\\\", :ipaddress => \\\"' + pro_hostname
                + '\\\", :type => \"' + pro_type + '\\\", :zone => Zone.default_zone);'
                'e.update_authentication(:default => {:userid => \\\"' + pro_username +
                '\\\", :password => \"' + pro_password + '\\\"});'
                'e.authentication_check;')
    else:
        logger.error('Unrecognized version: {}'.format(version.current_stream()))
...
def set_server_roles_benchmark_memory_all():
...
def set_server_roles_benchmark_memory_cap_and_util(use_web_ui=False):
    """Sets server roles used for all C&U memory measurement benchmarks."""
    if use_web_ui:
        roles = init_server_roles(True)
...
def set_server_roles_benchmark_memory_idle(use_web_ui=False):
    """Turns on all server roles used for idle memory measurement benchmark including
    datbase_synchronization role."""
    if use_web_ui:
        roles = init_server_roles(True)
        set_server_roles(**roles)
    else:
...
def set_server_roles_benchmark_memory_smartstate():
    """Sets server roles for Provisioning and Smartstate memory measurement benchmarks."""
    yaml = get_yaml_config('vmdb')
...
def set_server_roles_smartstate_benchmark():
